...
Based on the paper and official implementation:
...
class TokenEmbedding(nn.Module):
    ...
    _FORWARD_BAD_ARGS_MESSAGE = (
        'Based on the arguments passed to the constructor of TokenEmbedding, {}'
    )
        ...
        for argname, argvalue, module in [
            ...
            if module is None:
                if argvalue is not None:
                    raise ValueError(
                        TokenEmbedding._FORWARD_BAD_ARGS_MESSAGE.format(
...
class FTTransformer(nn.Module):
    ...
    def make_parameter_groups(self) -> List[Dict[str, Any]]:
        ...
        zero_wd_group: Dict[str, Any] = {
            'params': set(
                itertools.chain(
                    get_parameters(self.token_embed.cls_embedding),
                    get_parameters(self.token_embed.cont_embeddings),
                    get_parameters(self.token_embed.cat_embeddings),
    ...
    def forward(self, x_cont: Optional[Tensor], x_cat: Optional[Tensor]) -> Tensor:
        ...
        x = self.token_embed(x_cont, x_cat)
        x = self.backbone(x)
        return x

    def forward_from_tokens(self, x: Optional[Tensor]) -> Tensor:
        x = self.backbone(x)
        return x