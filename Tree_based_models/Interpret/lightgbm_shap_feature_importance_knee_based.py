...
- --root_path: directory that contains the SHAP CSV:
      {root_path}/{cls_type}/{variable_type}/{subfolder}/XGBoost_shap_{variable_type}_all_iters_folds.csv
...
def parse_args():
    ...
    parser.add_argument(
        "--data_path",
        type=str,
        default="/LocalData1/daheo/UKB_FINAL/data/Step5_refilter_categorical_for_deeplearning.csv",
        help="Path to input CSV data (used only to get feature lists via select_data).",
    )
    parser.add_argument(
        "--root_path",
        type=str,
        default="/LocalData1/daheo/UKB_FINAL/results/Tree_based/Lightgbm_shap_plz",
        help="Directory that contains XGBoost SHAP CSVs.",
    )
    parser.add_argument(
        "--outdir",
        type=str,
        default="/LocalData1/daheo/UKB_FINAL/results/Tree_based/Lightgbm_shap_plz/Interpret",
...
get_category_col = [f'shap::cat_{c}' for c in category_col]
get_continue_col = [f'shap::cont_{c}' for c in continue_col]
...
for it in range(N_ITER):
    ...
    for fd in range(N_FOLD):
        ...
        attr_num = torch.tensor(fold_df[get_continue_col].to_numpy())

        if len(get_category_col) > 0:
            attr_cat = torch.tensor(fold_df[get_category_col].to_numpy())
            attr_tot = torch.cat((attr_num, attr_cat), dim=-1)
        else:
            attr_tot = attr_num
